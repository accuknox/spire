...
// Type aliases to avoid verbose package qualifications in tests.
type (
	TokenAuthConfig   = cvault.TokenAuthConfig
	CertAuthConfig    = cvault.CertAuthConfig
	AppRoleAuthConfig = cvault.AppRoleAuthConfig
	K8sAuthConfig     = cvault.K8sAuthConfig
	AuthMethod        = cvault.AuthMethod
)

const (
	TOKEN   = cvault.TOKEN
	CERT    = cvault.CERT
	APPROLE = cvault.APPROLE
	K8S     = cvault.K8S
)
