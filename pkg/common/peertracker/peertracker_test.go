...
func TestExitDetection(t *testing.T) {
	...
	require.NoError(t, conn.Info.Watcher.IsAlive(nil))

	// Should return an error once we're no longer tracking
	peer.disconnect()
	conn.Close()
	require.EqualError(t, conn.Info.Watcher.IsAlive(nil), "caller is no longer being watched")
	...
	switch runtime.GOOS {
	case "darwin":
		require.EqualError(t, conn.Info.Watcher.IsAlive(nil), "caller exit detected via kevent notification")
	...
	case "linux":
		require.EqualError(t, conn.Info.Watcher.IsAlive(nil), "caller exit suspected due to failed readdirent")
		require.Len(t, test.logHook.Entries, 2)
		firstEntry := test.logHook.Entries[0]
		require.Equal(t, logrus.WarnLevel, firstEntry.Level)
		require.Equal(t, "Caller is no longer being watched", firstEntry.Message)
	...
	case "windows":
		require.EqualError(t, conn.Info.Watcher.IsAlive(nil), "caller exit detected: exit code: 0")
		require.Len(t, test.logHook.Entries, 2)
		firstEntry := test.logHook.Entries[0]
		require.Equal(t, logrus.WarnLevel, firstEntry.Level)
		require.Equal(t, "Caller is no longer being watched", firstEntry.Message)
