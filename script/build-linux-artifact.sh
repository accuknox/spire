...
build_artifact() {
    ...
    local artifact="${OUTDIR}/spire-${TAG}-linux-${arch}.tar.gz"
    local checksum="${OUTDIR}/spire-${TAG}-linux-${arch}_sha256sum.txt"

    local extras_artifact="${OUTDIR}/spire-extras-${TAG}-linux-${arch}.tar.gz"
    local extras_checksum="${OUTDIR}/spire-extras-${TAG}-linux-${arch}_sha256sum.txt"
